 ...
 * @returns {{}}
 */
// swagger.json的path项，每一个元素的键名都是一个接口的path
  ...
  for (var i = 0; i < apiPaths.length; i++) {
    ...
    for (var j = 0; j < verbs.length; j++) {
      ...
      if (type == 'post' || type == 'patch' || type == 'put') {
        _.extend(obj, createOutput(verb, swagger.definitions, pathKeys))
      } else {
        _.extend(obj, createOutput(verb, swagger.definitions))
      }
    }
  }
  return paths
...
function createVerbDefinitions(verbs, definitions) {
  ...
  if (verbs && verbs.success && verbs.success.fields) {
    fieldArrayResult = createResponseFieldArrayDefinitions(
...
function createResponseFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
  var result = {
    topLevelRef: topLevelRef,
    topLevelRefType: null,
  }
  ...
  for (var i = 0; i < fieldArray.length; i++) {
    ...
    definitions[objectName] = definitions[objectName] || { properties: {}, required: [], type: 'object' }
 ...
 * @returns {{}}
 */
function createOutput(verbs, definitions) {
  var pathItemObject = {}
...
function createPathParameters(verbs, pathKeys) {
  ...
  if (verbs.parameter && verbs.parameter.fields.Body) {
    let schema = { type: 'object', properties: {} }
    for (var i = 0; i < verbs.parameter.fields.Body.length; i++) {
      var param = verbs.parameter.fields.Body[i]
      var field = param.field
      var type = param.type
      schema.properties[param.field] = {
